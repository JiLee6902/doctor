...
let getUserPage = (req, res) => {
    ...
    let selectedMonth = parseInt(req.query.monthAnalyse) || currentMonth;
...
let getPostsWithPagination = async (req, res) => {
    try {
        ...
        let limit = +process.env.LIMIT_GET_POST || 5; 

        let object = await supporterService.getPostsPagination(page, limit, role);
...
let postBookingDoctorPageWithoutFiles = async (req, res) => {
    try {
        ...
        item.placeId = (item.places === 'none') ? null : item.places;
