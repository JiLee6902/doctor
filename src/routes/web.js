...
let initRoutes = (app) => {
    ...
    router.get('/doctor/manage/appointment', auth.checkLoggedIn, async (req, res) => {
        try {
            await doctor.getManageAppointment(req, res);
        } catch (error) {
            console.error('Error in manage appointment route:', error);
    ...
    router.delete('/admin/delete/feedback', auth.checkLoggedIn, admin.deleteFeedbackById);
